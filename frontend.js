...
const notificationButton = document.createElement('button');
  notificationButton.className = 'nav-link'; notificationButton.dataset.view = 'notifications'; notificationButton.innerHTML = '<span class="ui-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg></span> Notification <i id="notification-count" hidden>0</i>';
...
const mobileNotificationButton = document.createElement('button');
  mobileNotificationButton.dataset.view = 'notifications'; mobileNotificationButton.innerHTML = '<span class="ui-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg></span>Notification';
...
(() => {
  ...
  loadNotifications = async function() { if (!token) return; try { const {notifications,unread,unreadMessages}=await api('/notifications'); const count=$('#notification-count'); count.textContent=unread; count.hidden=!unread; const mobile=$('.bottom-nav [data-view="notifications"]'); if(mobile) mobile.innerHTML=`<span class="ui-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg></span>Notification${unread?`<i class="mobile-notification-count">${unread}</i>`:''}`; $('#notification-list').innerHTML=notifications.map(item=>`<button type="button" class="notification ${item.read?'read':'unread'}" data-notification-id="${item._id}" data-notification-type="${item.type}" data-notification-actor="${esc(item.actor?.username || '')}" data-notification-post="${item.post?._id||item.post||''}" data-notification-conversation="${item.conversation?._id||item.conversation||''}"><div class="avatar avatar-gold">${initials(item.actor?.name)}</div><p><b>${esc(item.actor?.name||'Someone')}</b> ${item.type==='follow'?'started following you':item.type==='like'?'liked your post':item.type==='comment'?'commented on your post':'sent you a message'}<small>${when(item.createdAt)}</small></p></button>`).join('')||'<p class="empty-profile">You have no notifications yet.</p>'; } catch(error){console.warn(error);} };
})();
// Reactions and profile reply history are direct interactions so they remain
// available after dynamic message/profile rendering.
(() => {
  const decorateMessages = () => {
    if (!activeChat) return;
    document.querySelectorAll('.x-message').forEach((node, index) => {
      const message = activeChat.messages?.[index];
      if (!message?._id || node.dataset.reactChat) return;
      node.dataset.reactChat = `${activeChat._id}:${message._id}`;
      node.classList.add('reactable');
      const count = (message.reactions || []).length;
      if (count) node.insertAdjacentHTML('beforeend', `<small class="reaction-count">♥ ${count}</small>`);
    });
  };
  new MutationObserver(decorateMessages).observe(document.body, { childList:true, subtree:true });
  const react = async node => {
    const [scope, id] = (node.dataset.reactChat || node.dataset.reactGroup || '').split(':');
    if (!scope || !id || node.dataset.reacting) return;
    node.dataset.reacting = '1'; node.classList.toggle('reacted');
    try {
      const path = node.dataset.reactChat ? `/conversations/${scope}/messages/${id}/react` : `/groups/${scope}/messages/${id}/react`;
      const result = await api(path, { method:'POST' });
      let badge = node.querySelector('.reaction-count');
      if (result.reactions) { if (!badge) { badge=document.createElement('small'); badge.className='reaction-count'; node.append(badge); } badge.textContent=`♥ ${result.reactions}`; }
      else badge?.remove();
    } catch (error) { node.classList.toggle('reacted'); toast(error.message); }
    finally { delete node.dataset.reacting; }
  };
  let hold;
  document.addEventListener('pointerdown', event => { const node=event.target.closest('[data-react-chat],[data-react-group]'); if (node) hold=setTimeout(()=>react(node), 650); }, true);
  document.addEventListener('pointerup', () => { clearTimeout(hold); hold=null; }, true);
  document.addEventListener('pointercancel', () => { clearTimeout(hold); hold=null; }, true);
  document.addEventListener('dblclick', event => { const node=event.target.closest('[data-react-chat],[data-react-group]'); if (node) { event.preventDefault(); react(node); } }, true);

  document.querySelectorAll('[data-profile-tab]').forEach(button => button.addEventListener('click', () => {
    if (button.dataset.profileTab !== 'replies') return;
    const username = (viewedProfile || me)?.username;
    const replies = posts.flatMap(post => (post.comments || []).filter(comment => comment.replyTo && comment.author?.username === username).map(comment => ({ post, comment })));
    $('#profile-posts').innerHTML = replies.map(({post,comment}) => `<article class="comment profile-reply"><p><b>${esc(comment.author?.name || 'User')}</b> replied: ${esc(comment.text)}</p><small>On ${esc(post.author?.name || 'a user')}’s post</small></article>`).join('') || '<p class="empty-profile">No replies yet.</p>';
  }, true));
})();
// Comment toggles must win over older document-level handlers.  Listening on
// window during capture means the control survives every feed/profile rerender.
window.addEventListener('click', event => {
  const toggle = event.target instanceof Element ? event.target.closest('[data-comment-toggle]') : null;
  if (!toggle) return;
  const thread = document.getElementById(`comments-${toggle.dataset.commentToggle}`);
  if (!thread) return;
  event.preventDefault();
  event.stopImmediatePropagation();
  thread.hidden = !thread.hidden;
  toggle.setAttribute('aria-expanded', String(!thread.hidden));
}, true);
...
(() => {
  ...
  document.addEventListener('submit', async event => { const form = event.target.closest('[data-group-message]'); if (!form) return; event.preventDefault(); const id = form.dataset.groupMessage, input = form.querySelector('input'), text = input.value.trim(); if (!text) return; input.value = ''; try { await api(`/groups/${id}/messages`, { method: 'POST', body: JSON.stringify({ text }) }); await openGroup(id); } catch (error) { input.value = text; toast(`Message was not sent: ${error.message}`); } }, true);
  ...
  setInterval(() => { if (document.visibilityState === 'visible') refreshSession(); }, 60 * 1000);
...
// Theme is intentionally left untouched here: it is a user preference, not session data.
...
(() => {
  ...
  const unreadCount = c => (c.messages||[]).filter(message => !mine(message) && !message.readAt).length;
  ...
  const search = async value => { const query=value.trim().toLowerCase(); if(!query){renderInbox();return;} const local=conversations.filter(c=>`${otherMember(c).name} ${otherMember(c).username}`.toLowerCase().includes(query)); $('#x-conversation-list').innerHTML=local.map(c=>`<div class="x-conversation" role="button" tabindex="0" data-x-chat="${c._id}">${avatar(otherMember(c))}<span><b>${esc(otherMember(c).name)}</b><small>@${esc(otherMember(c).username)}</small><p>${esc(preview(c.messages?.at(-1)))}</p></span></div>`).join(''); try{const {users}=await api('/users/search/'+encodeURIComponent(query));const unseen=users.filter(u=>!conversations.some(c=>otherMember(c).username===u.username));if(unseen.length)$('#x-conversation-list').insertAdjacentHTML('beforeend',`<p class="x-search-heading">People</p>${unseen.map(u=>`<div class="x-conversation" role="button" tabindex="0" data-start-dm="${esc(u.username)}">${avatar(u)}<span><b>${esc(u.name)}</b><small>@${esc(u.username)}</small><p>Start a conversation</p></span></div>`).join('')}`);}catch{} };
  loadChats = async function(){const result=await api('/conversations');const before=new Set(knownIncoming);conversations=(result.conversations||[]).sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt));conversations.forEach(c=>(c.messages||[]).filter(m=>!mine(m)&&!m.readAt).forEach(m=>{const key=`${c._id}:${m._id}`;if(knownIncoming.size&& !before.has(key) && Notification.permission==='granted')new Notification(otherMember(c).name,{body:preview(m),tag:key});knownIncoming.add(key);}));if(activeChat)activeChat=conversations.find(c=>c._id===activeChat._id)||null;renderMessages();};
  openChat = async id => { activeChat=conversations.find(c=>c._id===id)||null;if(!activeChat)return;show('messages');persist();renderMessages();if(unreadCount(activeChat)){try{await api(`/conversations/${id}/read`,{method:'POST'});await loadChats();}catch(error){toast(error.message);}} };
  ...
  renderMessages(); if(token) loadChats().catch(()=>{}); setInterval(()=>{if(token)loadChats().catch(()=>{});},25000);
