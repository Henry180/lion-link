...
(() => {
  ...
  const renderInbox = () => { const target=$('#x-conversation-list'); if(!target)return; target.innerHTML=conversations.map(c=>{const other=otherMember(c),last=c.messages?.at(-1),unread=unreadCount(c);return `<button class="x-conversation ${activeChat?._id===c._id?'selected':''} ${unread?'unread':''}" type="button" data-x-chat="${c._id}">${avatar(other)}<span><b>${esc(other.name)}</b><small>@${esc(other.username)}</small><p>${esc(preview(last))}</p></span><span class="x-conversation-meta"><time>${last?.createdAt?when(last.createdAt):''}</time>${unread?`<i>${unread>99?'99+':unread}</i>`:''}</span></button>`;}).join('')||'<p class="x-empty">No messages yet. Search for a Lion Link member to start one.</p>'; updateBadge(); };
  ...
  const search = async value => { const query=value.trim().toLowerCase(); if(!query){renderInbox();return;} const local=conversations.filter(c=>`${otherMember(c).name} ${otherMember(c).username}`.toLowerCase().includes(query)); $('#x-conversation-list').innerHTML=local.map(c=>`<button class="x-conversation" data-x-chat="${c._id}">${avatar(otherMember(c))}<span><b>${esc(otherMember(c).name)}</b><small>@${esc(otherMember(c).username)}</small><p>${esc(preview(c.messages?.at(-1)))}</p></span></button>`).join(''); try{const {users}=await api('/users/search/'+encodeURIComponent(query));const unseen=users.filter(u=>!conversations.some(c=>otherMember(c).username===u.username));if(unseen.length)$('#x-conversation-list').insertAdjacentHTML('beforeend',`<p class="x-search-heading">People</p>${unseen.map(u=>`<button class="x-conversation" data-start-dm="${esc(u.username)}">${avatar(u)}<span><b>${esc(u.name)}</b><small>@${esc(u.username)}</small><p>Start a conversation</p></span></button>`).join('')}`);}catch{} };
