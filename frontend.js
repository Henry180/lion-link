...
function verifiedBadge(user){return user?.role==='admin'?'<span class="verified" title="Verified Lion Link account" aria-label="Verified Lion Link account">✓</span>':'';}
...
(() => {
  ...
  document.addEventListener('click', async event => {
    ...
    if (follow) { event.preventDefault(); event.stopImmediatePropagation(); if (follow.dataset.followBusy) return; follow.dataset.followBusy = '1'; follow.disabled = true; try { const result = await api(`/users/${encodeURIComponent(follow.dataset.follow)}/follow`, { method:'POST' }); follow.textContent=result.following?'Following':'Follow'; follow.classList.toggle('following',result.following); followStates.set(follow.dataset.follow, result.following); if(result.following) followedUsernames.add(follow.dataset.follow); else followedUsernames.delete(follow.dataset.follow); if (viewedProfile?.username === follow.dataset.follow) { viewedProfile.isFollowing=result.following; viewedProfile.followers=result.followers; renderProfile(viewedProfile); } } catch(error) { toast(error.message); } finally { delete follow.dataset.followBusy; follow.disabled=false; } return; }
...
postMarkup = function(post) {
  ...
  if (followedUsernames.has(username)) markup = markup.replace(`<button class="follow-small" data-follow="${username}">Follow</button>`, `<button class="follow-small following" data-follow="${username}">Following</button>`);
